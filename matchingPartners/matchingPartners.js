...
function getOfficeAddresses(officesArr = []) {
  return officesArr.filter(office => office.officeDistanceFromLondon < 100000).map(item => item.address)
}
...
function getPartners(data = []) {
  return data.map(partner => {
    ...
    return {
      ...
      addresses: getOfficeAddresses(partner.offices)
    }
  })
